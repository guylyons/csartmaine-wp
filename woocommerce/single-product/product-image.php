...
<div class="images">
	...
	woocommerce_show_product_sale_flash(); // move the woocommerce sale flash here.
	// this is for absolute positioning..

	$attachment_count = count( $product->get_gallery_attachment_ids() );
	$gallery          = $attachment_count > 0 ? '[product-gallery]' : '';
	$props            = wc_get_product_attachment_props( get_post_thumbnail_id(), $post );
	$image            = get_the_post_thumbnail( $post->ID, apply_filters( 'product_thumb', 'shop_single' ), array(
	    'title'	 => $props['title'],
	    'alt'    => $props['alt'],
	) );
